sap.ui.define([
		...
		function getFrameUrl (sHash, sUrlParameters) {
			var sUrl = jQuery.sap.getResourcePath("plan/app", ".html");
			sHash = sHash || "";
			sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

			if (sHash) {
				sHash = "#Planit!-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
			} else {
				sHash = "#Planit!-display";
			}

			return sUrl + sUrlParameters + sHash;
		}

		return Opa5.extend("plan.test.integration.pages.Common", {
			...
			getMockServer : function () {
				return new Promise(function (success) {
					Opa5.getWindow().sap.ui.require(["plan/localService/mockserver"], function (mockserver) {
